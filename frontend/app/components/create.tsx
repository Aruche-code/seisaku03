...
const Create = ({ loading, setLoading, setImages }: CreateType) => {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    for (let i = 0; i < count; i++) {
      if (!seed) {
        // シードが指定されていない場合は、ランダムなシードを生成
        seedList.push(Math.floor(Math.random() * 100000000));
      } else {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(`画像生成に失敗しました: ${errorData.detail}`);
        setLoading(false);
        return;
      }
    ...
    <>
      ...
      <form onSubmit={onSubmit}>
        <div className="p-4 rounded-lg bg-[#E6F2FF] shadow">
          ...
          {/* ネガティブプロンプト */}
          <div className="mb-5">
            <div className="font-bold mb-2 text-sm">ネガティブ プロンプト</div>
            <textarea
              className="w-full border rounded-lg p-2 focus:outline-none bg-gray-50 focus:bg-white"
              rows={3}
              ref={negativeRef}
          ...
          {/* 画像生成数 */}
          <div className="mb-5">
            <div className="font-bold mb-2 text-sm">生成枚数</div>
            <div className="px-2">
              <Slider
                min={1}
                max={MAX_IMAGE_COUNT}
          ...
          <div className="mb-5">
            <div className="flex justify-between">
              <div className="font-bold mb-2 text-sm">生成サイズ</div>
          ...
          {/* 画像がプロンプトにどれだけ従うか */}
          <div className="mb-5">
            <div className="font-bold mb-2 text-sm">スケール</div>
            <input
              className="w-full border rounded-lg p-2 focus:outline-none bg-gray-50 focus:bg-white"
              type="number"
              step={0.5}
          ...
          {/* ステップ数 ノイズ除去 */}
          <div className="mb-5">
            <div className="font-bold mb-2 text-sm">ノイズ除去</div>
            <input
              className="w-full border rounded-lg p-2 focus:outline-none bg-gray-50 focus:bg-white"
              type="number"
              ref={stepsRef}
          ...
          {/* シード */}
          <div className="mb-5">
            <div className="font-bold mb-2 text-sm">シード生成</div>
            <input
              className="w-full border rounded-lg p-2 focus:outline-none bg-gray-50 focus:bg-white"
              type="number"
              ref={seedRef}
